...
export default function GanttView({ projectId, apiBase }: { projectId: number; apiBase: string }) {
  ...
  const [heightPerRow] = useState<number>(34)
  const [labelWidth, setLabelWidth] = useState<number>(260)
  const headerH = 40
  ...
  useEffect(() => {
    ...
    setLabelWidth(Math.min(420, Math.max(200, Math.ceil(max) + 24)))
  }, [rows])

  const totalDuration = data?.duration || 0
  const width = labelWidth + totalDuration * scale + 80
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#0f0f0f', border: '1px solid #2b2b2b', padding: '4px 8px', borderRadius: 8 }}>
          <Button size="small" onClick={() => setScale((s) => Math.max(8, Math.round(s * 0.8)))}>−</Button>
          <div style={{ minWidth: 76, textAlign: 'center', color: '#c9d7ef' }}>{scale} px/ед.</div>
          <Button size="small" onClick={() => setScale((s) => Math.min(80, Math.round(s * 1.25)))}>+</Button>
        </div>
      </div>

      <div ref={containerRef} style={{ width: '100%', overflow: 'auto', borderRadius: 8, border: '1px solid #2b2b2b', background: '#0f131a' }}>
        <svg width={Math.max(width, containerRef.current?.clientWidth || 0)} height={height} style={{ display: 'block' }}>
          <defs>
          ...
          <g transform={`translate(0, ${headerH})`}>
            {rows.map((n, idx) => (
              <g key={n.id}>
                <text x={12} y={idx * heightPerRow + heightPerRow * 0.66} fill="#c9d7ef" fontSize={13} fontFamily="Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial">#{n.id} · {n.name}</text>
                <text x={labelWidth - 8} y={idx * heightPerRow + heightPerRow * 0.66} textAnchor="end" fill="#7c8fa9" fontSize={12} fontFamily="Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial">
                  ES {n.es} · EF {n.ef}
                </text>
              </g>
            ))}
            ...
            {rows.map((n, idx) => {
              ...
              return (
                <g key={n.id} filter={isCrit ? 'url(#crit-glow)' : 'url(#soft-shadow)'}>
                  ...
                  <text x={x + 10} y={y + h / 2 + 4} fill={s.text} fontWeight={700} fontSize={12} fontFamily="Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial">
                    {n.duration} ед.
        ...
        <div ref={labelMeasureRef} style={{ position: 'absolute', visibility: 'hidden', pointerEvents: 'none' }}>Measure</div>
      </div>
