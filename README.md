...
```
backend/
  docs/               # Wiki-стиль документации по backend (архитектура, тесты, мониторинг)
  app/
    main.py
    db.py
    models.py
...
- Подробное описание тестов backend — в `backend/README.md` (раздел **"Тестирование backend (pytest)"**).
- Основные моменты:
  - интеграционные тесты лежат в `backend/tests/` и ходят по реальному API backend через HTTP;
  - есть тематические файлы тестов: базовый API, чаты и статусы задач, проекты и участники, массовые сценарии;
  - нагрузочные тесты помечены маркером `@pytest.mark.load` и запускаются отдельной командой `pytest -m load` (при запущенном Docker‑backend);
  - один из нагрузочных сценариев бесконечно повышает конкуренцию (логины, создание пользователей/проектов/задач, сообщения, анализ графа) и падает при первой серьёзной ошибке (HTTP 5xx или исключение), что удобно наблюдать через Grafana/Prometheus.

## Wiki / доп. документация

- Дополнительные материалы оформлены в виде небольшой wiki в каталоге `backend/docs/`:
  - `overview.md` — общее описание и сценарии использования;
  - `backend-architecture.md` — архитектура и слои backend;
  - `testing.md` — структура тестов и способы запуска;
  - `monitoring-and-load-testing.md` — детали по метрикам, Grafana/Prometheus и нагрузочным тестам.
